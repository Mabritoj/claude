...
- `template.yaml` is the root/orchestrating stack, scoped to SAM-native resources (`AWS::Serverless::Function`, etc.) plus `AWS::CloudFormation::Stack` declarations that wire in nested stacks. Plain CloudFormation resources (S3 buckets, CloudFront distributions, ACM certs, DynamoDB tables, IAM roles not auto-generated by SAM, etc.) don't belong directly in the root — they go in a service-grouped file under `infrastructure/` (e.g. `cloudfront.yaml` holding the distribution + origin bucket + cert + DNS record together). Flag anything that looks like a plain CFN resource sitting in the root template.
...
- A resource defined directly in `template.yaml` that looks like it belongs in `infrastructure/` instead (breaks the nested-stack convention).
